with source as (
    select * from {{ source('zoomcamp', 'green_tripdata') }}
),
...
renamed as (
    select
        ...
        {{ safe_cast('ehail_fee', 'NUMERIC') }} AS ehail_fee,  -- Green taxis have an ehail fee, but it may be null, so we use safe_cast to handle that
